...
void doEdge(int gpio_pin_num, int edge) {
    ...
    if ((fd = fopen("/sys/class/gpio/export", "w")) == NULL) {
        ...
        sprintf(errorMessage, "Unable to open GPIO export interface: %s", strerror(errno));
        HandleError(Dart_NewApiError(errorMessage));
    }
    ...
    if ((fd = fopen(fName, "w")) == NULL) {
        char errorMessage[256];
        sprintf(errorMessage, "Unable to open GPIO direction interface for pin %i: %s", gpio_pin_num, strerror(errno));
        HandleError(Dart_NewApiError(errorMessage));
    }
    ...
    if ((fd = fopen(fName, "w")) == NULL) {
        char errorMessage[256];
        sprintf(errorMessage, "Unable to open GPIO edge interface for pin %i: %s", gpio_pin_num, strerror(errno));
        HandleError(Dart_NewApiError(errorMessage));
    }
    ...
    if (edge == INT_EDGE_NONE) fprintf(fd, "none\n");
        ...
        sprintf(errorMessage,
                "Invalid edge mode specified for pin %i: Should be none, rising, falling or both. (%s)",
                gpio_pin_num,
                strerror(errno)
        );
        HandleError(Dart_NewApiError(errorMessage));
...
static const int interruptToPinMax = 16;
...
void gpioInterrupt10(void) { gpioInterrupt(interruptToPin[10]); }
void gpioInterrupt11(void) { gpioInterrupt(interruptToPin[11]); }
void gpioInterrupt12(void) { gpioInterrupt(interruptToPin[12]); }
void gpioInterrupt13(void) { gpioInterrupt(interruptToPin[13]); }
void gpioInterrupt14(void) { gpioInterrupt(interruptToPin[14]); }
void gpioInterrupt15(void) { gpioInterrupt(interruptToPin[15]); }
void gpioInterrupt16(void) { gpioInterrupt(interruptToPin[16]); }
